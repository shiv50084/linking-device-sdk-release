// Last Update:2019-06-13 19:40:42
/**
 * @file sim_ipc.c
...
static void * sim_ipc_video_task( void *instance )
{
    ...
    while ( ipc->running && p_video_buf < p_end ) {
        ...
        if ( p_video_buf + frame_len < p_end ) {
            ...
            if ( ipc->video_cb ) {
                ipc->video_cb( (uint8_t *)p_video_buf, frame_len, !(type == 1), timestamp, 0 );
            } else {
            }
        ...
        if ( offset + 7 <= len ) {
            ...
            if ( offset + size + var.aac_frame_length <= len ) {
                if ( ipc->audio_cb ) {
                    ...
                    ipc->audio_cb( (uint8_t *)buf_ptr + offset, var.aac_frame_length, timeStamp, 0 );
                }
            }
...
static int sim_ipc_capture_picture( ipc_dev_t *dev, char *path, char *file )
{
